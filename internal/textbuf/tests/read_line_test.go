...
func TestReadEmptyLine(t *testing.T) {
	...
	assert.Equal(t, "", std.IterToStr(buf.Read2(0, 0, 1, 0)))
...
func Test2Lines(t *testing.T) {
	...
	assert.Equal(t, "", std.IterToStr(buf.Read2(1, 0, 2, 0)))
...
func Test3Lines(t *testing.T) {
	...
	assert.Equal(t, "0\n", std.IterToStr(buf.Read2(0, 0, 1, 0)))
	assert.Equal(t, "1\n", std.IterToStr(buf.Read2(1, 0, 2, 0)))
	assert.Equal(t, "", std.IterToStr(buf.Read2(2, 0, 3, 0)))
...
func TestReadLineAtValidIndex(t *testing.T) {
	...
	assert.Equal(t, "dolor\n", std.IterToStr(buf.Read2(2, 0, 3, 0)))
	assert.Equal(t, "sit\n", std.IterToStr(buf.Read2(3, 0, 4, 0)))
	assert.Equal(t, "amet,\n", std.IterToStr(buf.Read2(4, 0, 5, 0)))
	assert.Equal(t, "consectetur\n", std.IterToStr(buf.Read2(5, 0, 6, 0)))
	assert.Equal(t, "adipiscing\n", std.IterToStr(buf.Read2(6, 0, 7, 0)))
	assert.Equal(t, "elit,\n", std.IterToStr(buf.Read2(7, 0, 8, 0)))
	assert.Equal(t, "sed\n", std.IterToStr(buf.Read2(8, 0, 9, 0)))
	assert.Equal(t, "do\n", std.IterToStr(buf.Read2(9, 0, 10, 0)))
	assert.Equal(t, "eiusmod\n", std.IterToStr(buf.Read2(10, 0, 11, 0)))
	assert.Equal(t, "tempor\n", std.IterToStr(buf.Read2(11, 0, 12, 0)))
	assert.Equal(t, "incididunt\n", std.IterToStr(buf.Read2(12, 0, 13, 0)))
	assert.Equal(t, "ut\n", std.IterToStr(buf.Read2(13, 0, 14, 0)))
	assert.Equal(t, "labore\n", std.IterToStr(buf.Read2(14, 0, 15, 0)))
...
func TestInsertAddsLines(t *testing.T) {
	...
	for i := 0; i < 10; i += 1 {
		...
		assert.Equal(t, fmt.Sprintf("%d\n", i), std.IterToStr(buf.Read2(i, 0, i+1, 0)))
		buf.Validate()
	}

	assert.Equal(t, 11, buf.LineCount())
	assert.Equal(t, "", std.IterToStr(buf.Read2(11, 0, 12, 0)))
	buf.Validate()
}
